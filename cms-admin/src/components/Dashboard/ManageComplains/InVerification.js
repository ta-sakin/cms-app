...
const InVerification = ({ complain, drawer }) => {
    ...
    try {
      if (!assigned) {
        ...
        const { data } = await axios.post("/admin/assign", {
          ...
          date_status: "in " + statusType + " start",
          complain_status: "in " + statusType,
          ...assign,
        });
  ...
  return (
    ...
    //   >
    //     <p className="text-white font-semibold py-2">Verification Completed?</p>
    //     <IoIosArrowForward
    ...
    <div className={`w-full max-w-sm ${drawer && "mx-auto"}`}>
      <div
        ...
        onClick={() => setExpand(!expand)}
      >
        <p className="text-white font-semibold py-2">Verificaiton Completed?</p>
        <IoIosArrowForward
          className={`duration-200 text-white ${
            expand && "transition-transform mb-2 origin-left rotate-90"
          }`}
      ...
      <div className="bg-gray-100 pb-8" hidden={!expand}>
        <div className="flex justify-center items-center gap-2 mx-3 pt-4">
          <p>Change status: </p>
