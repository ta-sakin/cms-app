...
const Rejected = ({ complain, drawer = false, setRefetchComplain }) => {
    ...
    (async () => {
      try {
        ...
        const { data } = await axios.get(
          `/admin/assign?cid=${complain._id}&status=rejected`
        );
        ...
        if (Object.keys(data).length < 1) {
          ...
          setUpdated(true);
          setLoading(false);
        } else if (data.status_type === "rejected") {
          setUpdated(true);
  ...
  return (
    <div className={`w-full max-w-sm ${drawer && "mx-auto"}`}>
      ...
      >
        ...
        >
          <p className="text-white font-semibold py-2">Rejection Details</p>
          <IoIosArrowForward
        ...
        <div className="pb-8" hidden={!expand}>
          <p className="mx-6 my-4">
            Rejected on:{" "}
            {moment(assignedStatus["in rejected start"]).format(
              "D MMM, YYYY hh:mm a"
            )}
          </p>
          <div className="mx-6 ">
